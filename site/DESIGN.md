...
The home demo uses the real plugin with fake actions and an isolated EventTarget. It never calls a store. Start with one illustrative tote so the per-line controls are immediately visible. Display actual lines, quantities, properties, line totals, and a subtotal, not operation tabs. Cart controls use plain-language labels. View code replaces the cart in place with a short, copyable set of independent add, update, remove, and gift-wrap calls. View cart returns to the same cart state. There is no modal, file viewer, or implementation dump. Both views share a grid cell so toggling preserves the panel footprint; the inactive view is hidden and inert. Thin controller wrappers handle status messages and focus. Keep the example product labelled, preserve empty/pending/disabled/success/failure states, and restore keyboard focus when a line is removed or merged. The cart region grows up to a bounded height on desktop while mobile content flows naturally.
