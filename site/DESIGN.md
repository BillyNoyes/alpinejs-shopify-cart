...
The home demo uses the real plugin with fake actions and an isolated EventTarget. It never calls a store. Start with one illustrative tote so the per-line controls are immediately visible. Display actual lines, quantities, properties, line totals, and a subtotal, not operation tabs. Cart controls use plain-language labels. A separate View code button opens a native modal dialog with copyable cart markup, controller, local transport, and setup. Source is taken from the implementation rather than maintained as a separate example; the viewer itself is omitted from the extracted markup. Thin controller wrappers handle status messages and focus. Keep the example product labelled, preserve empty/pending/disabled/success/failure states, and restore keyboard focus when a line is removed or merged. The cart region grows up to a bounded height on desktop while mobile content flows naturally.
