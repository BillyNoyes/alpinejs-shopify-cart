...
The home demo imports the repository's actual plugin source and supplies an in-memory runtime via `createPlugin`. It never contacts Shopify and does not expose store identifiers or credentials. View code switches the same panel to short, independent API examples; it does not expose implementation files or open a modal. The cart remains mounted with its state intact. Tests execute the examples to verify their payloads and enforce a short code sample. Documentation is static HTML with Alpine used only for navigation filtering, active-section tracking, and copy controls.
